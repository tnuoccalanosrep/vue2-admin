<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <p v-if="scope.row.expireType == 'fix'">
            {{ parseTime(scope.row.beginTime) }}
          </p>
          <p v-if="scope.row.expireType == 'fix'">
            {{ parseTime(scope.row.endTime) }}
          </p>
          <p v-if="scope.row.expireType == 'flex'">
          ...
          <el-col :span="24">
            <el-form-item label="过期类型" prop="expireType">
              <el-radio-group v-model="form.expireType">
                ...
                <el-radio key="flex" label="flex" value="flex"
                  >领取后生效</el-radio
                >
              </el-radio-group>
            </el-form-item>
            ...
            <el-col :span="24">
              <el-form-item label="发放方式" prop="sendWay">
                <el-radio-group v-model="form.sendWay">
                  <el-radio key="backend" label="backend" value="backend"
                    >后台发放</el-radio
                  >
                  <el-radio key="front" label="front" value="front"
                    >前台领取</el-radio
                  >
                  <el-radio key="offline" label="offline" value="offline"
                    >线下发放</el-radio
                  >
                </el-radio-group>
              </el-form-item>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item label="满多少可用" prop="outRule">
                <el-input
                  v-model="form.outRule"
                  style="width: 300px"
                  placeholder="单位：元，输入0表示无门槛"
                ></el-input>
              </el-form-item>
            </el-col>
          </el-row>
            ...
            <el-col :span="24">
              <el-form-item label="适用商品" prop="applyGoods">
                <el-radio-group v-model="form.applyGoods">
                  <el-radio key="allGoods" label="allGoods" value="allGoods"
                    >全场通用</el-radio
                  >
                  <el-radio key="parkGoods" label="parkGoods" value="parkGoods"
                    >指定商品</el-radio
                  >
                </el-radio-group>
              </el-form-item>
            ...
            <el-col :span="24" v-if="form.applyGoods == 'parkGoods'">
              <el-form-item label="适用商品ID" prop="goodsIds">
                <el-input
                  ...
                  rows="2"
                  type="textarea"
                  placeholder="英文逗号分隔，如：19932,938423,324234"
                ></el-input>
              </el-form-item>
            </el-col>
          </el-row>
            ...
            <el-col :span="24">
              <el-form-item label="使用专项" prop="useFor">
                <el-radio-group v-model="form.useFor">
                  ...
                  >
                    <el-option
                      v-for="useForItem in couponUseForList"
                      :key="useForItem.key + ''"
                      :label="useForItem.name"
                      :value="useForItem.key + ''"
                    />
                  </el-select>
                </el-radio-group>
              </el-form-item>
    ...
    <el-dialog
      title="发放卡券"
      :visible.sync="openSendCouponDialog"
      class="common-dialog"
      width="800px"
      append-to-body
    >
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="发放对象">
              <el-radio-group
                v-model="sendForm.object"
                @change="objectChange()"
              >
                <el-radio key="part" label="part" value="part"
                  >部分会员</el-radio
                >
                ...
                <el-button
                  type="danger"
                  size="mini"
                  v-if="sendForm.object == 'all'"
                  >总会员数：{{ totalMember }}</el-button
                >
                <el-button
                  type="primary"
                  size="mini"
                  v-if="sendForm.object == 'part'"
                  @click="selectUser()"
                  >选择会员（已选{{ sendForm.memberIds.length }}人）</el-button
                >
              </el-radio-group>
            </el-form-item>
          </el-col>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" v-loading="sendLoading" @click="doSendCoupon"
          >确定发放</el-button
        >
...
export default {
      ...
      if (this.form.type == 'P') {
        if (this.storeItem.length > 0) {
          ...
          this.storeItem.forEach(function (item) {
            if (
              item.storeAmount == '' ||
              isNaN(parseFloat(item.storeAmount)) ||
              item.giveAmount == '' ||
              isNaN(parseFloat(item.giveAmount))
            ) {
              validCount++
            }
          })
  ...
  methods: {
    ...
    handleStatusChange(row) {
      ...
      this.$modal
        .confirm('确认要' + text + 'ID等于' + row.id + '的数据项吗？')
        .then(function () {
    ...
    submitForm: function () {
      ...
      app.$refs['form'].validate((valid) => {
        if (valid) {
          app.form.storeIds = app.form.storeIds
            ? app.form.storeIds.join(',')
            : ''
          app.form.gradeIds = app.form.gradeIds
            ? app.form.gradeIds.join(',')
            : ''
          ...
          if (storeRules.length > 0) {
            storeRules.forEach(function (rule) {
              ...
              if (ruleItem.length == 2) {
                app.storeItem.push({
                  storeAmount: ruleItem[0],
                  giveAmount: ruleItem[1],
                })
              }
            })
          }
    ...
    handleDelete(row) {
      this.$modal
        .confirm('是否确认删除ID等于' + row.id + '的数据项？')
        .then(function () {
        ...
        if (valid) {
          app
            ...
            .then(() => {
              ...
              const userIds = app.sendForm.memberIds
                ? app.sendForm.memberIds.join(',')
                : ''
              const param = {
                couponId: app.sendForm.couponId,
                userIds: userIds,
                object: app.sendForm.object,
                num: app.sendForm.num,
                remark: app.sendForm.remark,
              }
              sendCoupon(param)
                .then((response) => {
                  if (response.data) {
