  ...
  <div class="sku-list">
      ...
      >
        <div class="sku-list-item-main">
          <div class="sku-list-item__layout">
            ...
            <el-input size="small" v-model="item.name" class="input"></el-input>
          </div>
          <div class="sku-list-item__layout">
    ...
    <el-table border :data="skuData.skuList">
      ...
      <el-table-column label="sku编码" align="center" width="160">
        <template slot-scope="scope">
          <el-input :readonly="disabled" v-model="scope.row.skuNo"></el-input>
        </template>
      </el-table-column>
      <el-table-column label="销售价格(元)" align="center">
        <template slot-scope="scope">
          <el-input :readonly="disabled" v-model="scope.row.price"></el-input>
        </template>
      </el-table-column>
  ...
  methods: {
    ...
    addSkuRow(i) {
      this.$prompt('请输入规格名称', '添加规格', {
        ...
        saveSpecName({ goodsId: this.goodsId, name: value }).then(
          (response) => {
    ...
    removeSkuRow(i) {
      ...
      app.$modal.confirm('确认删除该规格吗？').then(function () {
        deleteSpec({
          goodsId: app.goodsId,
          specName: app.skuData.attrList[i].name,
        }).then((response) => {
    ...
    removeSkuAttr(a, b) {
      ...
      app.$modal.confirm('确认删除该规格值吗？').then(function () {
        deleteSpecValue({ id: app.skuData.attrList[a].child[b].id }).then(
          (response) => {
    ...
    addSkuAttr(i) {
      this.$prompt('请输入规格值', '添加规格值', {
        ...
        saveSpecValue({
          ...
          specName: this.skuData.attrList[i].name,
          value: value,
        }).then((response) => {
