...
export default {
  ...
  data() {
    return {
      ...
      // 上传地址
      uploadAction: process.env.VUE_APP_SERVER_URL + 'backendApi/goods/goods/uploadGoodsFile',
      // 隐藏上传
      hideUpload: false,
      // 查询参数
      queryParams: {
  ...
  methods: {
    ...
    handleUploadSuccess(data) {
      if (data && data.code == '200') {
          this.getList();
          this.$modal.msgSuccess('商品导入成功');
      } else {
          this.$modal.msgError((data && data.message) ? data.message : '商品导入失败');
      }
      this.uploadGoods = [];
