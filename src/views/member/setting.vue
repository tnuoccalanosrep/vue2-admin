      ...
      >
        <el-row>
          <el-col :span="24">
            <el-form-item label="领券是否需要手机号" prop="getCouponNeedPhone">
              <el-select
                v-model="form.getCouponNeedPhone"
                placeholder="领券是否需要手机号"
              >
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item
              label="提交订单是否需要手机号"
              prop="submitOrderNeedPhone"
            >
              <el-select
                v-model="form.submitOrderNeedPhone"
                placeholder="提交订单是否需要手机号"
              >
                <el-option key="true" label="是" value="true" />
                <el-option key="false" label="否" value="false" />
              </el-select>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="登录是否需要手机号" prop="loginNeedPhone">
              <el-select
                v-model="form.loginNeedPhone"
                placeholder="登录是否需要手机号"
              >
                <el-option key="true" label="是" value="true" />
                <el-option key="false" label="否" value="false" />
              </el-select>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="是否开通微信会员卡" prop="openWxCard">
              <el-select
                v-model="form.openWxCard"
                placeholder="是否开通微信会员卡"
              >
                <el-option key="true" label="是" value="true" />
                <el-option key="false" label="否" value="false" />
              </el-select>
              <el-button
                v-if="form.openWxCard == 'true'"
...
export default {
  ...
  data() {
    return {
      ...
      // 表单参数
      form: {
        getCouponNeedPhone: 'false',
        submitOrderNeedPhone: 'false',
        loginNeedPhone: 'false',
        openWxCard: 'false',
      },
      // 表单校验
      rules: {
        ...
        openWxCard: [{ required: true, message: '请选择', trigger: 'blur' }],
      },
    }
  },
  created() {
  ...
  methods: {
    ...
    submitForm: function () {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          const param = {
            getCouponNeedPhone: this.form.getCouponNeedPhone,
