  ...
  >
    ...
    <div class="member-info" v-if="memberInfo.id">
      ...
      <div class="item">
        <div class="value">
          可用余额：{{
            memberInfo.balance ? memberInfo.balance.toFixed(2) : '0.00'
          }}（元）
        </div>
        <div class="value">
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" v-if="!memberInfo.id" @click="doQuery()"
        >查询会员</el-button
      >
      <el-button v-if="memberInfo.id" @click="cancel()">重新查询</el-button>
      <el-button type="danger" v-if="memberInfo.id" @click="doSubmit()"
        >确认会员</el-button
      >
    </div>
  </el-dialog>
...
export default {
  ...
  methods: {
    doQuery() {
      ...
      if (keyword.length > 0) {
        getMemberInfo({ keyword: this.form.keyword.trim() }).then(
          (response) => {
